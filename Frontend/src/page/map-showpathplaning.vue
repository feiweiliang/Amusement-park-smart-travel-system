        ...
        <div class="pr area-wrap oh">
            ...
            <div class="layer mapFloat">
                <template v-if="!item.block" v-for="(item,index) in polygons">
                    <span :key="'float'+index" class="_title" :style="{left:item.attrDataX+'px', top:item.attrDataY+'px'}"><h2>{{item.attrTitle}}</h2><br/></span>
                </template>
            </div>
            <!-- 路径绘制层 -->
        ...
        methods: {
            ...
            useTest: function () {
               // 这是导入地图的方式
                ...
                console.log("sessionStorage.getItem('pathxxx')",sessionStorage.getItem("pathxxx"));
                // 导入路径的方式
               var path2= JSON.parse(sessionStorage.getItem("pathxxx"));  
               console.log(typeof(path2) );
                //  this.importPath(sessionStorage.getItem("path"));
                
            //   'http://127.0.0.1:8181/path/getRoutePlan/830/'+path
            // sessionStorage.getItem("pathxxx")
               this.importPath(path2);
                // instance.get('static/jpath2.json').then(res => {
